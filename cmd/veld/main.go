...
import (
	...
	_ "github.com/Adhamzineldin/Veld/internal/emitter/backend/nest"
...
func printImportInstructions(rc config.ResolvedConfig) {
	...
	if hasBackend {
		...
		case "nestjs":
			fmt.Println(dim("    Setup:") + ` run ` + bold("veld setup") + ` then ` + bold("npm install"))
			fmt.Println(dim("    Types:      ") + ` import type { User } from '@veld/generated/types';`)
			fmt.Println(dim("    Controller: ") + ` import { UsersController } from '@veld/generated/controllers/users.controller';`)
			fmt.Println(dim("    Services:   ") + ` import type { IUsersService } from '@veld/generated/interfaces/IUsersService';`)
		case "python":
...
Backends:  node-ts, node-js, nestjs, python, go, rust, java, csharp, php
Frontends: typescript, javascript, react, vue, angular, svelte, dart, kotlin, swift, types-only, none
Aliases:   node → node-ts, js/javascript → node-js, nest/nest.js → nestjs, ts → typescript, react → react-hooks`,
		SilenceUsage:  true,
		SilenceErrors: true,
	}
...
func newGenerateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Generates typed backend interfaces and a frontend SDK from your .veld contract.\n\n" +
			...
			"Aliases:   node → node-ts, js/javascript → node-js, nest/nest.js → nestjs",
...
func runInit() error {
	...
	if selectedBackend == "node-ts" || selectedBackend == "node-js" || selectedBackend == "nestjs" || selectedBackend == "python" {
