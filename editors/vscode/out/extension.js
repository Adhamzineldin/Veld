...
exports.VeldLanguageServer = void 0;
...
class VeldLanguageServer {
                    ...
                    while (j < lines.length && lines[j].trim() !== '}') {
                        ...
                        if (actionLine.startsWith('action')) {
                            if (inAction && currentAction.name) {
                                actions.set(currentAction.name, currentAction);
                            }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmed.startsWith('import')) {
                ...
                if (importMatch) {
                    ...
                    if (!isUsed) {
                        diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, 0, i, trimmed.length), `Import '${importPath}' is not used`, vscode.DiagnosticSeverity.Warning));
                    }
                }
                else if (trimmed.match(/import\s+/)) {
                    // Invalid import syntax
                    diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, 0, i, trimmed.length), `Invalid import syntax. Use: import @alias/name (e.g., import @models/user)`, vscode.DiagnosticSeverity.Error));
                }
            }
            ...
            if (trimmed.startsWith('input:') || trimmed.startsWith('output:')) {
                ...
                for (const match of typeMatches) {
                    ...
                    if (!veld_language_spec_1.BUILTIN_TYPES.has(typeName) && !doc.models.has(typeName) && !doc.enums.has(typeName)) {
                        ...
                        diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, match.index, i, match.index + typeName.length), msg, vscode.DiagnosticSeverity.Error));
                    }
                }
            }
            ...
            if (!trimmed.startsWith('method:') &&
                ...
                if (fieldMatch) {
                    ...
                    if (!veld_language_spec_1.BUILTIN_TYPES.has(typeName) && !doc.models.has(typeName) && !doc.enums.has(typeName)) {
                        ...
                        diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, trimmed.indexOf(typeName), i, trimmed.indexOf(typeName) + typeName.length), msg, vscode.DiagnosticSeverity.Error));
                    }
                }
            }
            // Check HTTP methods
            if (trimmed.startsWith('method:')) {
                ...
                if (methodMatch && !veld_language_spec_1.HTTP_METHODS.has(methodMatch[1])) {
                    ...
                    diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, methodStart, i, methodStart + methodMatch[1].length), `Invalid HTTP method '${methodMatch[1]}'. Valid: GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS`, vscode.DiagnosticSeverity.Error));
                }
            }
            ...
            if (dirMatch && trimmed.length > 0 && !trimmed.startsWith('//')) {
                ...
                if (directive[0] !== directive[0].toLowerCase() ||
                    (!veld_language_spec_1.DIRECTIVES.has(directive) && !veld_language_spec_1.KEYWORDS.has(directive))) {
                    // Only warn if it looks like a directive (not a field)
                    if (directive[0] === directive[0].toLowerCase() && directive.length > 0) {
                        // It's a field, skip
                    }
                    else if (!veld_language_spec_1.DIRECTIVES.has(directive) && !veld_language_spec_1.KEYWORDS.has(directive)) {
                        diagnostics.push(new vscode.Diagnostic(new vscode.Range(i, 0, i, directive.length), `Unknown directive '${directive}'. Valid: ${Array.from(veld_language_spec_1.DIRECTIVES).join(', ')}`, vscode.DiagnosticSeverity.Warning));
                    }
                }
            }
        }
    ...
    getCompletions(uri, position, content) {
        ...
        if (beforeCursor.includes(':')) {
            for (const type of veld_language_spec_1.BUILTIN_TYPES) {
        ...
        if (beforeCursor.includes('action') || beforeCursor.includes('module')) {
            for (const directive of veld_language_spec_1.DIRECTIVES) {
...
exports.VeldLanguageServer = VeldLanguageServer;
