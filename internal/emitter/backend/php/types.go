...
func (e *PhpEmitter) emitModels(a ast.AST, outDir string) error {
	...
	// Organise into per-module subdirectories with matching namespaces.
	// Use `use App\Models\Auth\User;` for precise imports.
	modelGroups, enumGroups, _, _ := emitter.AssignModelsToModules(a)
	keys := emitter.SortedGroupKeys(modelGroups, enumGroups)

	for _, group := range keys {
		groupPascal := phpPascalName(group)
		subDir := filepath.Join(outDir, "app", "Models", groupPascal)
		if err := os.MkdirAll(subDir, 0755); err != nil {
			return err
		}
		ns := "App\\Models\\" + groupPascal

		for _, en := range enumGroups[group] {
			if err := e.emitEnum(en, subDir, ns); err != nil {
				return fmt.Errorf("enum %s: %w", en.Name, err)
			}
		}
		for _, m := range modelGroups[group] {
			if err := e.emitClass(m, enumNames, subDir, ns); err != nil {
...
func (e *PhpEmitter) emitClass(m ast.Model, enumNames map[string]bool, dir, ns string) error {
...
func (e *PhpEmitter) emitEnum(en ast.Enum, dir, ns string) error {
	var sb strings.Builder
	sb.WriteString("<?php\n\n")
	sb.WriteString("// AUTO-GENERATED BY VELD \u2014 DO NOT EDIT\n\n")
	sb.WriteString("declare(strict_types=1);\n\n")
