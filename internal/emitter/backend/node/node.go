...
// ── types/{module}.ts — per-module files, each type defined exactly once ──────
//
...
func (e *NodeEmitter) emitPerModuleTypes(a ast.AST, outDir string) error {
	...
	// key=typeName, value=moduleLowerName (the file that defines it).
	...
	for _, mod := range a.Modules {
		...
		// Separate into types we define vs types we re-export from another file.
		...
		// Re-export imports from other module files.
