...
func emitJavaSdkModels(consumed emitter.ConsumedServiceInfo, sdkDir, pkg string) error {
	...
	// Build: modelName → first module that uses it (lowercase, for subfolder name)
	...
	for _, en := range a.Enums {
		modLower := enumOwner[en.Name]
		if modLower == "" {
			continue // unused enum
		}
		subDir := filepath.Join(sdkDir, "models", modLower)
		if err := os.MkdirAll(subDir, 0755); err != nil {
			return err
		}
		subPkg := fmt.Sprintf("maayn.veld.generated.sdk.%s.models.%s", pkg, modLower)
	...
	for _, m := range a.Models {
		modLower := modelOwner[m.Name]
		if modLower == "" {
			continue // unused model
		}
		subDir := filepath.Join(sdkDir, "models", modLower)
		if err := os.MkdirAll(subDir, 0755); err != nil {
			return err
		...
		if m.Extends != "" {
			...
			if parentMod != "" && parentMod != modLower {
