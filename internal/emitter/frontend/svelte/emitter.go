...
func (e *SvelteEmitter) Emit(a ast.AST, outDir string, opts emitter.EmitOptions) error {
	...
	// Write stores/package.json with per-module exports
	...
	for _, mod := range a.Modules {
		...
		pkg.WriteString(fmt.Sprintf(",\n    \"./%s.store\": \"./%s.store.ts\"", modLower, modLower))
	}
	pkg.WriteString("\n  }\n}\n")
	return os.WriteFile(filepath.Join(storesDir, "package.json"), []byte(pkg.String()), 0644)
}
