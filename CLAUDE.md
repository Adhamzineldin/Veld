...
module ModuleName {
  ...
  action ActionName {
    ...
    input: ModelName
    output: ModelName
    output: ModelName:201       // explicit success status code (optional)
    output: ModelName[]:200     // array output with status code
    query: QueryModel
    headers: {
...
- **Correct HTTP status codes**: POST → 201, DELETE with no output → 204, else 200; overridable via `output: Type:NNN`
